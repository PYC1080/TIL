...
```
1. 리눅스의 사용자 시스템 : 다중 사용자 시스템, 1대의 리눅스에 사용자 여러 명이 동시에 접속해서 사용할 수 있는 시스템
2. 사용자 패스워드 확인
	1) 위치 : vi or vim or gedit  /etc/passwd
	2) 형태 : 사용자이름:암호:사용자ID:사용자소속그룹ID:전체이름:홈디렉터리:기본셀
		(1) 암호 
			a) x : /etc/shadow 파일에 비밀번호가 저장되어 있다
		(2) 사용자소속그룹 : /etc/group
            a) 위치 : /etc/group
            b) 형태 : 그룹이름:비밀번호:그룹id:그룹에속한_사용자_이름
```

#### (2) 사용자 및 그룹과 관련된 명령어

##### a) useradd or adduser : 새로운 사용자를 추가한다

```
1. 의미 : 새로운 사용자를 추가하거나 기존 유저 정보를 업데이트 한다. 별도의 그룹을 지정하지 않으면 자도응로 사용자 이름과 동일한 그룹이 생성되고 새로운 사용자는 생성된 그룹에 자동으로 포함된다
2. 용법 : useradd 옵션 사용자
3. 옵션
	1) -b, --base-dir : 만약 -d 옵션으로 홈 디렉터리가 지정되지 않으면 기존 디렉터리를 제공한다
	2) -c, --comment : 로그인시 짧은 설명을 표시한다
	3) -d, --home-dir :  새로운 유저가 사용할 홈디렉터리 값을 지정한다
	4) -D, --defaults : 기존 유저 정보를 업데이트 하는데 사용하는 옵션
	5) -e, --expiredate : 지정한 날짜에 계정이 만료되도록 값을 넣는다
	6) -f, --inactive : 정해진 날짜가 지난 후 비밀번호가 만료되며 계정이 영구적으로 사용 중지된다
	7) -g, --gid :  그룹 이름 또는 유저의 최초 로그인 그룹의 숫자
	8) -G, --groups : 
	9) -h, --help :
	10) -k, --skel :
	11) -K, --key :
...
##### b) passwd : 사용자의 비밀번호를 지정하거나 변경한다

```
1. 의미 : 사용자의 비밀번호를 지정하거나 변경한다
2. 용법 : passwd [-k] [-l] [-u[-f]] [-d] [-e] [-n mindays] [-x maxdays] [-w warndays] [-i inactivedays] [-S] [--stdin] [username]
3. 옵션
	1) -k, --keep : 만료된 패스워드의 업데이트 할 때 사용하는 옵션
	2) -l, --lock : 해당 옵션을 사용하면 해당 계정이 잠긴다. 해당 계정은 루트 계정만 접근할 수 있다
	3) --stdin :
    4) -u, --unlock : 
    5) -d, --delete : 
    6) -f, --force : 
...
##### e) change : 사용자의 암호를 주기적으로 변경하도록 설정한다

```
1. 의미 : 사용자의 암호를 주기적으로 변경하도록 설정한다
2. 용법 : change 옵션 
3. 옵션
	1)
```

##### f) groups : 사용자가 소속된 그룹을 보여준다

```
1. 의미 : 사용자가 소속된 그룹을 보여준다
2. 용법 : groups 사용자명
```

##### g) groupadd :  새로운 그룹을 생성한다
...
##### j) gpasswd : 그룹의 암호를 설정하거나 그룹관리를 수행한다

```
1. 의미 : 그룹의 암호를 설정하거나 그룹관리를 수행한다
2. 용법 : gpasswd 옵션
3. 옵션
	1) -a, --add :
	2) -d, --delete :
...
```
-rw-r--r--. 1 root root         0  3월 16 17:02 sample.txt
1. 파일 유형 : -
2. 파일 허가권 : rw-r--r--
3. 링크 수 : 1 
4. 파일 소유자 이름 : root
