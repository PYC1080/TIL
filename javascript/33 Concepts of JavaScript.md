...
```
1. 표현식은 문장처럼 동작할 수 있다.

2. 표현식은 값을 만들어 낸다 : 표현식은 값 하나로 귀결되는 자바스크립트 코드 조각(snippet)이다.

3. 표현식은 반드시 상태를 바꿀 필요는 없다.
```

### 2) 문장, Statements

```
1. 문장은 표현식처럼 동작할 수 없다.

2. 문장은 무언가를 수행한다.

3. 문장은 자바스크립트에서 값이 들어와야 할 곳에 들어갈 수 없다. 따라서 함수의 인자로도, 대입 연산의 값으로도, 연산자의 피연산자로도 사용될 수 없다.
```

### 3) 함수 선언, 함수 표현식 그리고 네임드 함수 표현식
...
console.log(foo(function funcName(){})); // "funcName"

자바스크립트에서 값이 들어가는 곳에서 스크립트, 모듈, 블록 문장의 전역 단계에 함수를 선언하는 것은 함수 선언이다. 
...
```
표현식 뒤에 세미콜로만 추가하면 표현식을 표현식 문장으로 바꿀 수 있다.
...
```
1. 세미콜론을 붙이면 여러 줄의 문장을 하나의 줄에 넣을 수 있다.

2. 콤마 연산자는 우리가 여러 개의 표현식을 연결할 수 있도록 도와준다. 단, 반환은 마지막 표현식만 반환된다.
```
...
```
익명 함수는 표현식으로만 쓰일 수 있다. 따라서 자바스크립트에서 값이 들어갈 곳에 쓰일 수 있다면 괄호 안에 익명 함수를 값으로 넘길 수 있다. 이때 익명 함수를 괄호 속에 넣는다면 즉시 같은 익명 함수를 리턴한다.
```
