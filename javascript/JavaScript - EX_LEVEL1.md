...
function solution (n,arr1,arr2){
    ...
    for(let i=0;i<n;i++){
        ...
        for(let j=n;j>0;j--){
            let arr1Check = false
            let arr2Check = false
            if(Math.pow(2,(j-1))<=arr1No){
                arr1Check = true
                arr1No -= Math.pow(2,(j-1))
            }
            
            if(Math.pow(2,(j-1))<=arr2No){
                arr2Check = true
                arr2No -= Math.pow(2,(j-1))
            }
    
            if(arr1Check==false && arr2Check==false){
                answerCheck +=' '
            }else{
                answerCheck +='#'
            }
        }
        answer.push(answerCheck)
...
3. stages에는 1 이상 N + 1 이하의 자연수가 담겨있다.
	...
	1) 각 자연수는 사용자가 현재 도전 중인 스테이지의 번호를 나타낸다.
	2) 단, N + 1 은 마지막 스테이지(N 번째 스테이지) 까지 클리어 한 사용자를 나타낸다.
...
function solution(N,stages){
    ...
    stageData.sort(sortFunction)
    for(let d=0; d<stageData.length;d++){
        answer.push(stageData[d][0])
    }
    return answer
}
...
function solution(data){
    ...
    for(let i=0;i<dataArray.length;i++){
        if(isNaN(dataArray[i])==false){
            a++;
            if(dataArray[i]==1&&dataArray[i+1]==0){
                no[a].push(10);
                i++
            }else{
                no[a].push(Number(dataArray[i]))
            }
        }else{
            no[a].push(dataArray[i])
        }
    }
    console.log(no)
    for(let j=0;j<3;j++){
